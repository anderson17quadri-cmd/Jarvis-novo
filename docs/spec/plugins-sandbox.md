...
| `core.storage.set/get/remove` | `storage` | Armazenamento isolado, prefixo `plugins:<id>:`, derivado num único sítio (`plugin-storage.ts`) — `chave` vazia recusada à entrada |

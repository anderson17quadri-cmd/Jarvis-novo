...
| Editor visual em blocos | ⬜ | **Avaliado 10/08/2026 — não bem-escopado, não começado.** A Parte 13 original pede um editor de nós a sério: dez tipos de bloco (Evento, Condição, Ação, **Loop, Espera, Variável, IA, Plugin**, Notificação, Fim), ligados visualmente, com zoom, pan, seleção múltipla, copiar/colar, agrupar. O motor de hoje (`automation-service.ts`) é `Gatilho → Condições[] → Ações[]`, uma lista plana — sem ramos, sem loops, sem variáveis, sem passos encadeados. Um editor de nós sobre um motor plano seria interface a fingir: a mesma razão por que o Terminal e a execução de plugins continuam de fora. Três decisões por tomar antes de haver âmbito nenhum para escopar, nenhuma técnica: (1) "editor visual" quer dizer mesmo um canvas de nós livre, ou chegaria um construtor visual mais simples — menus e campos, sem desenhar ligações — sobre o modelo de dados que já existe? (2) se for o canvas a sério, o modelo de dados muda primeiro (gatilho→condições→ações plano vira um grafo com ramos e loops) — projeto à parte, maior do que o editor em si; (3) metade dos blocos do original (Plugin, muito do que webhook/API pedem) dependem de capacidades ainda bloqueadas nesta app (execução de plugins, rede real) — entram cinzentos, ou só entram os blocos que já executam a sério hoje? |

...
| Wake word configurável | 🟡 | **24.1 fechada (19/08/2026)** — motor local (Vosk `small-pt-0.3`, português) num serviço Python à parte (`wake-word-service/`), gerido pelo Rust com health-check a sério (carrega o modelo e valida o microfone antes de armar, não só a porta a responder). Testado com áudio real (silêncio, ruído, a palavra isolada, e dentro de outra frase) — ver `docs/log/historico-sessoes.md`. **Achado**: o modelo pequeno tem vocabulário fechado e nunca reconhece "Jarvis" — palavra por omissão passou a "Sentinela", que está no vocabulário. Falta 24.2 (ligação ao `voiceService`, interruptor, indicador) e 24.3 (persistência, auditoria). Desenho em [`docs/spec/wake-word-local.md`](docs/spec/wake-word-local.md) |

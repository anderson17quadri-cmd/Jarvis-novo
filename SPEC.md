...
| Stores separadas | 🟡 | 16 das 26. As 16 com serviço seguem o padrão (14 com subscrição direta na store, 2 por hook: `useAiSettings` → `aiService`, `useSystemMetrics` → `systemService`). Das 10 restantes, 9 são estado local puro sem fonte externa (janelas, widgets, tarefas, sessão, aparência, temas custom, assistente, correção de voz, navegação pendente) — não precisam de serviço. A que falta (Plugin) tem serviço em curso. `useDataService` já não tem consumidores |

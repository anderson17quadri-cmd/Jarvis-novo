...
| **Rede real** — pesquisa web | `services/web-search/providers/brave-search-provider.ts`, `services/web-search/providers/searxng-search-provider.ts` | ✅ **desbloqueado pelo utilizador 13/08/2026 (Brave), SearXNG a 20/08/2026 (item 28)** — três provedores com escolha explícita em Personalização → Pesquisa web (Simulado / SearXNG / Brave), por omissão o simulado. O SearXNG não pede chave nem conta — só o endereço de uma instância local (`http://localhost:8888` por omissão, único endereço na CSP) — mas a pergunta sai da máquina na mesma, dito por palavras na própria janela; instalar e correr o SearXNG é do utilizador. Os dois provedores reais vão por `fetch` da interface, nunca pelo `fetch_page_text` do Rust (que tem um bloqueio de SSRF contra `localhost` de propósito — mandar-lhe um endereço local seria recusado). A ferramenta `pesquisar_na_web` só devolve título, resumo e endereço por resultado — nunca HTML nem o conteúdo de nenhuma página — e não executa nada por si. **Revisão adversarial (20/08/2026, item 15)**: achado real corrigido — resultados reais chegavam ao modelo sem a defesa contra injeção de instruções que `abrir_pagina` já tinha; agora partilham a mesma (`src/lib/untrusted-content.ts`) |
...
| Widgets previstos | ✅ | Doze: Relógio, CPU, RAM, Disco, Rede, Clima, Notícias, Email, Música, **Calendário**, **Tarefas** e **IA**. O de Tarefas mexe nas mesmas tarefas da janela, e o de Calendário lê a mesma agenda. **GPU não entra**: ver §5. **Revisão adversarial de Clima/Notícias/Email/Música/Calendário, 20/08/2026 (item 15) — achado real e corrigido**: uma falha na sondagem (`PollingDataService`) só ia para a consola — nenhum dos cinco widgets alguma vez dizia "não consegui atualizar", e sem nenhuma leitura boa ainda, o widget ficava preso no esqueleto de carregamento para sempre. `WidgetError` (já existia, nunca usado por nenhum widget) passa a aparecer nesse caso, com "tentar novamente"; com dados antigos já mostrados, um aviso discreto fica ao lado deles. Ver `docs/log/historico-sessoes.md` |
...
| Wake word configurável | ✅ | **24.1–24.3 fechadas (19/08/2026)** — motor local (Vosk `small-pt-0.3`, português) num serviço Python à parte (`wake-word-service/`), gerido pelo Rust com health-check a sério. Testado com áudio real (silêncio, ruído, a palavra isolada, e dentro de outra frase) — ver `docs/log/historico-sessoes.md`. **Achado**: o modelo pequeno tem vocabulário fechado e nunca reconhece "Jarvis" — palavra por omissão passou a "Sentinela". Ligada ao `voiceService` (interruptor na Privacidade, indicador permanente no header), **exige o serviço local de voz a correr para se armar** (recusa e avisa, nunca cai para a nuvem à calada — provado a falhar sem o guard), palavra configurável com persistência, e cada acordar fica no `logService`. **Por confirmar**: uso ao vivo com microfone físico e ecrã real (esta sessão verificou por `tsc`/`eslint`/`vitest`/`cargo` e áudio sintetizado, não conseguiu tirar print da janela nativa). Desenho em [`docs/spec/wake-word-local.md`](docs/spec/wake-word-local.md) |
...
| **Gatilhos do sistema** (ficheiros, USB, bateria, rede) | 🟡 | Ficheiros ✅ (`notify` crate, `watch_folder`/`unwatch_folder`), USB ✅ (`windows` crate, poll 5s a `SetupDiGetClassDevsW`), Bateria ✅ (`battery` crate, poll 30s). Rede ⬜ — **por construir; a nota antiga dizia "sem API" e estava errada** (corrigido na auditoria de 20/08/2026): o `GetAdaptersAddresses` do crate `windows` serve, e o gatilho está construído e testado no repositório irmão `sexta-feira` (`src-tauri/src/commands/network.rs`) — é trazer, não inventar. Três comandos Rust: `watch_folder`, `unwatch_folder`, `get_battery_status`. Três capacidades novas no `PlatformAdapter`: `fileWatcher`, `usbMonitor`, `batteryMonitor`. Editor visual com blocos novos (Alteração de ficheiro, Dispositivo USB, Nível da bateria). Eventos emitem `automation://file-changed`, `automation://usb-changed`, `automation://battery-changed`; `checkNativeTriggers()` no `automationService` casa-os contra as regras ativas e regista no histórico de 60 execuções. **Revisto 13/08/2026** (revisão independente): corrigida uma fuga em `unwatch_folder` (a thread do observador nunca parava) e o cruzamento de limiar da bateria para mais do que uma regra ao mesmo tempo. **Revisão adversarial a sério do lado nativo, `battery.rs`/`usb.rs` (item 15, 20/08/2026) — nenhum dos dois tinha um teste sequer**: um achado real na bateria — uma leitura falhada do gestor de energia (`battery::Manager`, não falta de bateria, que já é tratada à parte) apagava o último estado bom; a leitura seguinte, mesmo idêntica à de antes da falha, era lida como "mudança" e disparava um evento a mais sem nada ter mudado de verdade. Corrigido preservando o último estado bom através de uma falha transitória (`current.or(last)`, não só `current`). A lógica de diferença de ambos (bateria e USB) foi extraída para funções puras testáveis sem hardware nenhum — 10 testes novos, o da bateria confirmado a apanhar o achado (falha sem a correção, passa com ela) |

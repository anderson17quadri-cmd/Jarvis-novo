...
| Voz clonada local (a própria voz, via XTTS-v2) | 🟡 | `voice-clone-service/` — serviço Python à parte (GPU confirmada — RTX 5070), fora do que o Tauri/browser cobre sozinho. `setup.ps1`/`run.ps1` automatizam a instalação e o arranque — só a gravação da voz continua manual, por natureza. Rotas testadas com um modelo simulado; **o XTTS-v2 a sério nunca correu, sem GPU disponível aqui** — falta confirmar no PC. Só a voz de quem usa o sistema — nunca a de terceiros. Desenho em [`docs/spec/voz-clonada-local.md`](docs/spec/voz-clonada-local.md) |

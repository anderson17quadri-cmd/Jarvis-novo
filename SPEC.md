...
| Serviços desacoplados | 🟡 | 10 dos 14: System, AI, Voice, Storage, Theme, Notification, Workspace, Weather, Clock, Wallpaper. O Workspace usa `WorkspaceStores` + `getWorkspaceStores()` (injeção). O Notification ganhou a opção `silent` e perdeu o `useAssistantStore`. Weather, Clock e Wallpaper ganharam serviço + store Zustand (padrão `ThemeService`/`useThemeStore`). Os restantes (Calendar, News, Email, Search, Device, Plugin) são Fase 2 |
| Stores separadas | 🟡 | 10 das 18. A `use-ai-settings-store` é agora só estado + persistência; o hook `useAiSettings` aplica ao `aiService`. Weather, Clock e Wallpaper seguem o padrão: serviço puro → store fina → componente lê da store |
...
| Comandos naturais compreendidos | ✅ | Com a DeepSeek ligada, **o modelo escolhe as ações** de um catálogo de 23 ferramentas, e encadeia-as: um pedido pode abrir uma janela, criar uma tarefa e mudar de tema de uma vez. Sem modelo, continua o interpretador de padrões |
...
| Contexto ("amanhã", "esse ficheiro") | 🟡 | **`criar_tarefa` ganhou `prazo`, 10-11/08/2026** — o modelo resolve a data (já sabe "hoje é terça-feira, 11 de agosto" pelo `system prompt`) e devolve AAAA-MM-DD; `tool-runner.ts` só valida a forma, nunca interpreta "amanhã" por regras próprias. Confirmado por 6 testes (`tests/assistant/tools.test.ts`, `tests/stores/task-store.test.ts`) — data válida vira o timestamp certo, ausente ou malformada fica sem prazo em vez de inventar um. **Não confirmado ao vivo de ponta a ponta nesta sessão**: ferramentas só correm com a DeepSeek (`ai-service.ts` — `sendWithTools` exige `DeepSeekProvider`), e esta máquina não tinha chave DeepSeek configurada; com Ollama ativo (sem ferramentas) o pedido chega ao modelo mas nunca invoca `criar_tarefa` a sério. "Esse ficheiro" continua por fazer — não há ainda nenhuma ferramenta que atue sobre um ficheiro para a referência fazer sentido |
...
| **Carregar e executar um plugin** | 🟡 | **Confirmado a sério 10-11/08/2026** — `plugins/runtime/`: `<iframe sandbox="allow-scripts">` sem `allow-same-origin`, protocolo tipado por `postMessage`, permissão real verificada por `handlePluginMessage()` antes de qualquer serviço. Quatro plugins de exemplo correm código isolado de verdade: notificações, ficheiros (pasta declarada, nunca o disco inteiro) e rede (domínio autorizado explicitamente), mais disparar automações existentes. Falta verificação de assinatura — não há ainda fonte de terceiros a sério, só o catálogo local. Desenho completo em [`docs/spec/plugins-sandbox.md`](docs/spec/plugins-sandbox.md) |
...
| API do Core para plugins | 🟡 | **9 de 13 capacidades (11/08/2026)**: notificações, ficheiros, rede, automações, comandos, janelas, eventos, **armazenamento isolado** (`core.storage.set/get/remove`, prefixo `plugins:<id>:` por plugin) e **atalhos de teclado** (`core.shortcut.register`, com validação contra atalhos reservados do sistema). As últimas duas entraram com dois plugins de exemplo (`guarda-preferencias`, `regista-atalho`) e a permissão nova correspondente em `PluginPermissions`. Restam quatro do original — Criar Widgets, Adicionar Menus, Adicionar Configurações, Criar Serviços, Executar Voz, Ler Memória, Adicionar Painéis (7) — sem plugin real a pedi-las ainda |
